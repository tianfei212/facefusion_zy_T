...
def render() -> None:
    ...
    DEEP_SWAPPER_MORPH_SLIDER = gradio.Slider(
        ...
        visible=has_deep_swapper
        and load_processor_module("deep_swapper").get_inference_pool()
        and load_processor_module("deep_swapper").has_morph_input(),
    )
...
def listen() -> None:
    ...
    DEEP_SWAPPER_MORPH_SLIDER.release(
        update_deep_swapper_morph, inputs=DEEP_SWAPPER_MORPH_SLIDER
    )
...
def remote_update(processors: List[str]) -> Tuple[gradio.Dropdown, gradio.Slider]:
    ...
    return gradio.Dropdown(visible=has_deep_swapper), gradio.Slider(
        visible=has_deep_swapper
        and load_processor_module("deep_swapper").get_inference_pool()
        and load_processor_module("deep_swapper").has_morph_input()
    )
...
def update_deep_swapper_model(
    deep_swapper_model: DeepSwapperModel,
) -> Tuple[gradio.Dropdown, gradio.Slider]:
    ...
    if deep_swapper_module.pre_check():
        return gradio.Dropdown(
            value=state_manager.get_item("deep_swapper_model")
        ), gradio.Slider(visible=deep_swapper_module.has_morph_input())
