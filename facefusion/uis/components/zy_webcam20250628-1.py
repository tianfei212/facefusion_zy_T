...
from facefusion.uis.components.zy_webcam_options import WEBCAM_INPUT_TYPE_RADIO, WEBCAM_DEVICE_ID_DROPDOWN, \
...
def listen() -> None:
	...
	"""
		为输入类型选择器添加事件监听，以控制其他组件的可见性。
		"""
	# # 【最终修正】直接使用在 render() 函数中实例化的全局组件变量。
	# # 这种方式最直接，可以避免在复杂的UI加载顺序中出现组件引用问题。
